...
from resources.libs import extract, downloader, notify, skinSwitch, yt, speedtest, wizard as wiz
...
def index():
	if AUTOUPDATE == 'Yes':
		if wiz.workingURL(WIZARDFILE) == True:
			...
			else: addFile('Addon [COLOR dodgerblue][B]%s[/COLOR] v%s' % (ADDONTITLE, VERSION), '', themeit=THEME2)
	...
	if len(BUILDNAME) > 0:
		...
		if not themefile == False:
			addFile('Ninguno' if BUILDTHEME == "" else BUILDTHEME, 'theme', BUILDNAME, themeit=THEME5)
	else: addDir('No Instalada', 'builds', themeit=THEME4)
	if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
	addDir ('Builds y Parches'        ,'builds',   icon=ICONBUILDS,   themeit=THEME1)
	...
	addDir ('Copias de seguridad'     ,'maint', 'backup', icon=ICONSAVE,     themeit=THEME1)
	if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
...
def buildMenu():
	...
	if not WORKINGURL == True:
		...
		addDir ('Conservar Datos Importantes'       ,'savedata', icon=ICONSAVE,     themeit=THEME3)
		if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
		...
		if total == 1 and third == False:
			for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
				...
				return
		addFile('%s Version: %s' % (MCNAME, KODIV), '', icon=ICONBUILDS, themeit=THEME3)
		addFile('[COLOR gold]Version GTKING Instalada:[/COLOR] %s %s' % (BUILDNAME, BUILDVERSION))              
		addDir ('Conservar Datos Importantes'       ,'savedata', icon=ICONSAVE,     themeit=THEME3)       
		if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
		if third == True:
			for item in addin:
		...
		if len(match) >= 1:
			if SEPERATE == 'true':
				...
				if count18 > 0:
					...
					addFile('[B]%s Builds GTKING (%s)[/B]' % (state, count18), 'togglesetting',  'show18', themeit=THEME3)
					if SHOW18 == 'false':
						for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
							if not SHOWADULT == 'true' and adult.lower() == 'yes': continue
							if not DEVELOPER == 'true' and wiz.strTest(name): continue
				...
				if count16 > 0:
					state = '[COLOR dodgerblue]+[/COLOR]' if SHOW16 == 'false' else '[COLOR dodgerblue]-[/COLOR]'
					addFile('[B]%s [COLOR dodgerblue]Parches GTKING (%s)[/COLOR][/B]' % (state, count16), 'togglesetting',  'show16', themeit=THEME3)
					if SHOW16 == 'false':
						for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
							if not SHOWADULT == 'true' and adult.lower() == 'yes': continue
							if not DEVELOPER == 'true' and wiz.strTest(name): continue
				...
				if count17 > 0:
					...
					addFile('[B]%s Gracias por Instalar GTKing Build (%s)[/B]' % (state, count17), 'togglesetting',  'show17', themeit=THEME3)
					if SHOW17 == 'false':
						for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
							if not SHOWADULT == 'true' and adult.lower() == 'yes': continue
							if not DEVELOPER == 'true' and wiz.strTest(name): continue
...
def viewBuild(name):
    ...
    for version, url, gui, kodi, themefile, icon, fanart, preview, adult, info, description in match:
        ...
        if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
        addDir ('Conservar Datos Importantes',       'savedata', icon=ICONSAVE,     themeit=THEME1)
        addDir ('Copias de Seguridad'     ,'maint', 'backup',   icon=ICONMAINT, themeit=THEME1)
        addFile('Mas informacion en Telegram [COLOR lime]t.me/beelinkking[/COLOR]',    'buildinfo', name, description=description, fanart=fanart, icon=icon, themeit=THEME1)       
        addFile(wiz.sep('INSTALACION'), '', fanart=fanart, icon=icon, themeit=THEME3)
        temp3 = int(float(KODIV)); temp4 = int(float(kodi))
        if not temp3 == temp4:
            if temp3 == 18 and temp4 <= 17: warning = False
            else: warning = False
        else: warning = True
        if warning == True:
            addFile('[COLOR lime][B]INSTALAR BUILD[/B][/COLOR]'   , 'install', name, 'fresh'  , description=description, fanart=fanart, icon=icon, themeit=THEME1)
        temp5 = int(float(KODIV)); temp6 = int(float(kodi))
        if not temp5 == temp6:
            if temp5 == 17 and temp6 <= 15: warning = False
            else: warning = True
        else: warning = False
        if warning == True:
            addFile('[COLOR dodgerblue][B]INSTALAR[/B][/COLOR]', 'install', name, 'normal' , description=description, fanart=fanart, icon=icon, themeit=THEME1)
...
def apkScraper(name=""):
    if name == 'kodi':
        ...
        addFile("Versiones oficiales de Kodi", themeit=THEME1)
        rc = False
...
def apkMenu(name=None, url=None):
    if url == None:
        addDir ('Kodi versiones oficiales Apk\'s', 'apkscrape', 'kodi', icon=ICONAPK, themeit=THEME1)
        if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
    if not APKFILE == 'http://':
        if url == None:
                    ...
                    if plugin.lower() == 'section':
                        ...
                        try:
                            ...
                            if os.path.exists(add):
                                aname   = "[COLOR springgreen][Instalado][/COLOR] %s" % aname
                        except:
                            pass
                        x += 1
...
def packInstaller(name, url):
    ...
    DP.create(ADDONTITLE,'[COLOR %s][B]Descargando:[/B][/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, name), '', '[COLOR %s]Espera[/COLOR]' % COLOR2)
    urlsplits = url.split('/')
            ...
            if len(match) > 0:
                for name, url, repository, repositoryxml, repositoryurl, icon, fanart, adult, description in match:
                    if os.path.exists(os.path.join(ADDONS, plugin)):
                        do        = ['Lanzar Addon', 'Borrar Addon']
                        selected = DIALOG.select("[COLOR %s]Addon ya instalado, que quieres hacer?[/COLOR]" % COLOR2, do)
                        if selected == 0:
                            ...
                            if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres borrar la carpeta addon_data de:" % COLOR2, "[COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR1, plugin), yeslabel="[B][COLOR springgreen]Borrar[/COLOR][/B]", nolabel="[B][COLOR red]Saltar[/COLOR][/B]"):
...
def maintMenu(view=None):
    ...
    if view == "clean" or SHOWMAINT == 'true':
        ...
        addFile('Limpiar paquetes: [COLOR springgreen][B]%s[/B][/COLOR]' % wiz.convertSize(sizepack),     'clearpackages',   icon=ICONMAINT, themeit=THEME3)
        addFile('Limpiar imagenes: [COLOR springgreen][B]%s[/B][/COLOR]' % wiz.convertSize(sizethumb),  'clearthumb',      icon=ICONMAINT, themeit=THEME3)
        if os.path.exists(ARCHIVE_CACHE): addFile('Limpiar Cache de Archivos: [COLOR springgreen][B]%s[/B][/COLOR]' % wiz.convertSize(archive), 'cleararchive',    icon=ICONMAINT, themeit=THEME3)
        addFile('Limpiar imagenes antiguas', 'oldThumbs',      icon=ICONMAINT, themeit=THEME3)
    ...
    addDir ('[B]AutoLimpieza[/B]'     ,'maint', 'autoclean',   icon=ICONMAINT, themeit=THEME1)
    ...
    addDir ('[B]Agrega tu wizard[/B]'     ,'maint', 'thirdparty',   icon=ICONMAINT, themeit=THEME1)
    if view == "thirdparty" or SHOWMAINT == 'true':
	...
	if thirdparty == 'true':
			...
			addFile('Agregar Wizard 1: [COLOR %s]%s[/COLOR]' % (COLOR2, first), 'editthird', '1', icon=ICONMAINT, themeit=THEME3)
			addFile('Agregar Wizard 2: [COLOR %s]%s[/COLOR]' % (COLOR2, secon), 'editthird', '2', icon=ICONMAINT, themeit=THEME3)
			addFile('Agregar Wizard 3: [COLOR %s]%s[/COLOR]' % (COLOR2, third), 'editthird', '3', icon=ICONMAINT, themeit=THEME3)
	addDir ('[I]<< Volver al Menu Principal GTKING[/I]', icon=ICONMAINT, themeit=THEME2)	    
	addFile('Abrir Menus: %s' % maint.replace('true',on).replace('false',off) ,'togglesting', 'showmaint', icon=ICONMAINT, themeit=THEME2)
	setView('files', 'viewType')
...
def speedTest():
    addFile('Empezar test de velocidad',             'runspeedtest',      icon=ICONMAINT, themeit=THEME3)
    if os.path.exists(SPEEDTESTFOLD):
        ...
        if len(speedimg) > 0:
            addFile('Limpiar Resultados',          'clearspeedtest',    icon=ICONMAINT, themeit=THEME3)
            addFile(wiz.sep('Test Anteriores'), '', icon=ICONMAINT, themeit=THEME3)
            for item in speedimg:
                ...
                addFile('[B]%s[/B]: [I]Realizado el %s[/I]' % (img, created), 'viewspeedtest', img, icon=ICONMAINT, themeit=THEME3)
...
def removeAddonMenu():
    ...
    if len(selected) > 0:
        ...
        else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres [COLOR %s]Forzar el cierre[/COLOR]de Kodi o [COLOR %s]Recargar el perfil[/COLOR]? La Opcion recomendada es CERRAR.[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR red]Recargar[/COLOR][/B]", nolabel="[B][COLOR spinggreen]Cerrar Kodi[/COLOR][/B]")
...
def removeAddonDataMenu():
    if os.path.exists(ADDOND):
        addFile('[COLOR red][B][BORRAR][/B][/COLOR] Toda la carpeta Addon_Data', 'removedata', 'all', themeit=THEME2)
        addFile('[COLOR red][B][BORRAR][/B][/COLOR] Addon_Data de Addons Desinstalados', 'removedata', 'uninstalled', themeit=THEME2)
        addFile('[COLOR red][B][BORRAR][/B][/COLOR] Carpetas vacias de Addon_Data', 'removedata', 'empty', themeit=THEME2)
        addFile('[COLOR red][B][BORRAR][/B][/COLOR] Carpeta %s de Addon_Data' % ADDONTITLE, 'resetaddon', themeit=THEME2)
        if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
        ...
        for folder in sorted(fold, key = lambda x: x):
            ...
            if foldername in EXCLUDES: folderdisplay = '[COLOR springgreen][B][BLOQUEADO][/B][/COLOR] %s' % folderdisplay
...
def enableAddons():
    addFile("[I][B][COLOR red]Atencion: Desactivar algunos addons puede causar problemas[/COLOR][/B][/I]", '', icon=ICONMAINT)
    fold = glob.glob(os.path.join(ADDONS, '*/'))
    x = 0
    for folder in sorted(fold, key = lambda x: x):
        foldername = os.path.split(folder[:-1])[1]
...
def buildWizard(name, type, theme=None, over=False):
    ...
    if type == 'gui':
        ...
        if yes:
            ...
            else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]The Gui fix has been installed.  Quieres Forzar el cierre de Kodi o Recargar el perfil?[/COLOR]" % COLOR2, yeslabel="[B][COLOR red]Recargar[/COLOR][/B]", nolabel="[B][COLOR springgreen]Cerrar Kodi[/COLOR][/B]")
        ...
        if warning == True:
            yes_pressed = DIALOG.yesno("%s - [COLOR red]ATENCION[/COLOR]" % ADDONTITLE, '[COLOR %s]Vas a instalar un Parche de la Build GTKing ' % COLOR2, 'Quieres instalar: [COLOR %s]%s v%s[/COLOR]?[/COLOR]' % (COLOR1, name, wiz.checkBuild(name,'version')), nolabel='[B][COLOR red]Cancelar[/COLOR][/B]',yeslabel='[B][COLOR springgreen]Instalar[/COLOR][/B]')
        else:
            if not over == False: yes_pressed = 1
            else: yes_pressed = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Estas a punto de instalar el [COLOR gold][B]Parche[/B][/COLOR]' % COLOR2, '[COLOR %s]%s v%s[/COLOR]?[/COLOR]' % (COLOR1, name, wiz.checkBuild(name,'version')), nolabel='[B][COLOR red]Cancelar[/COLOR][/B]',yeslabel='[B][COLOR springgreen]Instalar[/COLOR][/B]')
        if yes_pressed:
            wiz.clearS('build')
            ...
            if int(float(percent)) > 0:
                ...
                if int(float(errors)) > 0:
                    yes=DIALOG.yesno(ADDONTITLE, '[COLOR %s][COLOR %s]%s v%s[/COLOR]' % (COLOR2, COLOR1, name, wiz.checkBuild(name,'version')), 'Completedo: [COLOR %s]%s%s[/COLOR] [Errores:[COLOR %s]%s[/COLOR]]' % (COLOR1, percent, '%', COLOR1, errors), 'Quieres ver los errores?[/COLOR]', nolabel='[B][COLOR red]No[/COLOR][/B]', yeslabel='[B][COLOR springgreen]Si[/COLOR][/B]')
                    if yes:
                ...
                else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres [COLOR %s]CERRAR[/COLOR] Kodi o [COLOR %s]Recargar el perfil[/COLOR]? La opcion recomendada es CERRAR[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR red]Recargar[/COLOR][/B]", nolabel="[B][COLOR springgreen]Cerrar[/COLOR][/B]")
                if todo == 1: wiz.reloadFix()
...
def thirdPartyInstall(name, url):
    ...
    type = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Instalacion [COLOR %s]Con Limpieza (Builds)[/COLOR] o [COLOR %s]Sin Limpieza (Parches)[/COLOR] Para el paquete:[/COLOR]" % (COLOR2, COLOR1, COLOR1), "[COLOR %s]%s[/COLOR]" % (COLOR1, name), yeslabel="[B][COLOR springgreen]Con Limpieza[/COLOR][/B]", nolabel="[B][COLOR red]Sin Limpieza[/COLOR][/B]")
    if type == 1:
    ...
    if int(float(percent)) > 0:
        ...
        if int(float(errors)) > 0:
            yes=DIALOG.yesno(ADDONTITLE, '[COLOR %s][COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, name), 'Completedado: [COLOR %s]%s%s[/COLOR] [Errores:[COLOR %s]%s[/COLOR]]' % (COLOR1, percent, '%', COLOR1, errors), 'Quieres ver los errores?[/COLOR]', nolabel='[B][COLOR red]No[/COLOR][/B]',yeslabel='[B][COLOR springgreen]Ver Errores[/COLOR][/B]')
            if yes:
                if isinstance(errors, unicode):
                    error = error.encode('utf-8')
                wiz.TextBox(ADDONTITLE, error)
    ...
    else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres [COLOR %s]Forzar el cierre de[/COLOR] Kodi o [COLOR %s]Recargar el perfil[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR red]Recargar perfil[/COLOR][/B]", nolabel="[B][COLOR springgreen]Cerrar Kodi[/COLOR][/B]")
    if todo == 1: wiz.reloadFix()
    else: wiz.killxbmc(True)
...
def apkInstaller(apk, url):
    ...
    if wiz.platform() == 'android':
        yes = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres descargar e instalar:" % COLOR2, "[COLOR %s]%s[/COLOR]" % (COLOR1, apk), yeslabel="[B][COLOR springgreen]Descargar[/COLOR][/B]", nolabel="[B][COLOR red]Cancelar[/COLOR][/B]")
...
def removeAddonData(addon, name=None, over=False):
    if addon == 'all':
        if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Quieres eliminar [COLOR %s]TODOS[/COLOR] los datos de la carpeta Addon_data?[/COLOR]' % (COLOR2, COLOR1), yeslabel='[B][COLOR springgreen]Eliminar Data[/COLOR][/B]', nolabel='[B][COLOR red]No Eliminar[/COLOR][/B]'):
        ...
        if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Quieres eliminar [COLOR %s]TODOS[/COLOR] los datos de la carpeta Addon_data de addons desinstalados?[/COLOR]' % (COLOR2, COLOR1), yeslabel='[B][COLOR springgreen]Eliminar Data[/COLOR][/B]', nolabel='[B][COLOR red]No Eliminar[/COLOR][/B]'):
            total = 0
        ...
        if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Quieres eliminar [COLOR %s]TODAS[/COLOR] las carpetas vacias de userdata?[/COLOR]' % (COLOR2, COLOR1), yeslabel='[B][COLOR springgreen]Borrar Data[/COLOR][/B]', nolabel='[B][COLOR red]No Eliminar[/COLOR][/B]'):
        ...
        if addon in EXCLUDES:
            ...
            if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Quieres borrar de addon_data:[/COLOR]' % COLOR2, '[COLOR %s]%s[/COLOR]' % (COLOR1, addon), yeslabel='[B][COLOR springgreen]Eliminar Data[/COLOR][/B]', nolabel='[B][COLOR red]No eliminar[/COLOR][/B]'):
...
def freshStart(install=None, over=False):
    ...
    elif install == 'restore': yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres restaurar" % COLOR2, "Kodi a su configuracion inicial", "Antes de instalar el backup?[/COLOR]", nolabel='[B][COLOR red]Cancelar[/COLOR][/B]', yeslabel='[B][COLOR dodgerblue]Continuar[/COLOR][/B]')
    elif install: yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Kodi se restaurara a su configuracion inicial" % COLOR2, "eliminando todos los archivos antes de", "Instalar la Build Inicial [COLOR %s]%s[/COLOR]" % (COLOR1, install), nolabel='[B][COLOR red]Cancelar[/COLOR][/B]', yeslabel='[B][COLOR dodgerblue]Continuar[/COLOR][/B]')
    else: yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres restaurar" % COLOR2, "Kodi a su configuracion inicial? Se eliminara todo.[/COLOR]", nolabel='[B][COLOR red]Cancelar[/COLOR][/B]', yeslabel='[B][COLOR dodgerblue]Continuar[/COLOR][/B]')
    if yes_pressed:
        if not wiz.currSkin() in ['skin.confluence', 'skin.estuary']:
            ...
            xbmc.sleep(1000)
        if not wiz.currSkin() in ['skin.confluence', 'skin.estuary']:
            wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Instalacion Limpia: Error en cambio de skin[/COLOR]' % COLOR2)
            return
        wiz.addonUpdates('set')
        ...
        for root, dirs, files in os.walk(xbmcPath,topdown=True):
            ...
            for name in files:
                ...
                elif name == 'MyVideos116.db' and fold[-1] == 'Database' and KEEPMYVIDEOS == 'true': wiz.log("Keep MyVideos: %s" % os.path.join(root, name), xbmc.LOGNOTICE)
        ...
        if not install == 'restore':
            wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Limpieza e instalacion Canceladas[/COLOR]' % COLOR2)
            wiz.refresh()
...
####THANKS GUYS @ NaN #######
def clearCache():
    if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Quieres borrar el cache?[/COLOR]' % COLOR2, nolabel='[B][COLOR red]Cancelar[/COLOR][/B]', yeslabel='[B][COLOR springgreen]Borrar Cache[/COLOR][/B]'):
        wiz.clearCache()

def clearArchive():
    if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Quieres borrar la carpeta \'Archive_Cache\'?[/COLOR]' % COLOR2, nolabel='[B][COLOR red]Cancelar[/COLOR][/B]', yeslabel='[B][COLOR springgreen]Borrar[/COLOR][/B]'):
        wiz.clearArchive()

def totalClean():
    if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Quieres borrar el cache, los paquetes y thumbnails?[/COLOR]' % COLOR2, nolabel='[B][COLOR red]Cancelar[/COLOR][/B]',yeslabel='[B][COLOR springgreen]Borrar todo[/COLOR][/B]'):
        wiz.clearCache()
...
def clearThumb(type=None):
    ...
    else: choice = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Quieres borrar el archivo %s y las carpetas de thumbnails?' % (COLOR2, latest), "Se recargaran en el proximo inicio[/COLOR]", nolabel='[B][COLOR red]No borrar[/COLOR][/B]', yeslabel='[B][COLOR springgreen]Borrar[/COLOR][/B]')
    if choice == 1:
...
def purgeDb():
    ...
    if KODIV >= 16:
        choice = DIALOG.multiselect("[COLOR %s]Elige los archivos a Purgar[/COLOR]" % COLOR2, display)
        if choice == None: wiz.LogNotify("[COLOR %s]Purgar Base de Datos[/COLOR]" % COLOR1, "[COLOR %s]Cancelada[/COLOR]" % COLOR2)
        elif len(choice) == 0: wiz.LogNotify("[COLOR %s]Purgar Base de Datos[/COLOR]" % COLOR1, "[COLOR %s]Cancelada[/COLOR]" % COLOR2)
        else:
            for purge in choice: wiz.purgeDb(DB[purge])
    else:
        choice = DIALOG.select("[COLOR %s]Elige los archivos a Purgar[/COLOR]" % COLOR2, display)
        if choice == -1: wiz.LogNotify("[COLOR %s]Purgar Base de Datos[/COLOR]" % COLOR1, "[COLOR %s]Cancelada[/COLOR]" % COLOR2)
